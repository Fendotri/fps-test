...
const levelMapPath = `${(import.meta as any).env?.VITE_LEVEL_MAP || ''}`.trim() || '/levels/de_dust_2_with_real_light.glb';
...
export const initResource = async () => {
    ...
    const map = GameResources.loader.loadAsync(levelMapPath);
