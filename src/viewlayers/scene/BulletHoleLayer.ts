...
import { getRuntimeQualityProfile, scaleQualityCount, scaleQualityTime } from '@src/core/RuntimeQuality';
...
export class BulletHoleLayer implements CycleInterface, LoopInterface {
    ...
    maximun: number = scaleQualityCount(40, 12); // 最大产生弹点数量

    bulletHoleOpacity: number = .8; // 弹孔透明度
    bulletHoleScale: number = QUALITY.tier === 'low' ? 1.25 : 1.5; // 弹孔大小
    exitTime: number = scaleQualityTime(5, 1.8); // 弹孔存在时间(多少秒后开始渐变消失)
    fadeTime: number = scaleQualityTime(.5, .2); // 弹孔消失渐变时间
